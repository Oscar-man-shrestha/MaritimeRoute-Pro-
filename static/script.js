...
function displayResults(data) {
  ...
  // Only show weather if enabled AND we have weather data
  if (includeWeather && data.weather_recommendation && comparisonDiv) {
                    ...
                    <div class="weather-summary">
                        <div class="summary-content">
                            ...
                            <div class="summary-text">
                                <p><strong>${
                                  data.weather_recommendation
                                }</strong></p>
                                <div class="summary-stats">
                                    <span class="summary-stat">
                                        <strong>Best Weather:</strong> 
                                        <span class="${
                                          fastestWeather.average_impact <
                                          fuelWeather.average_impact
                                            ? "color-excellent"
                                            : "color-good"
                                        }">
                                            ${
                                    ...
                                    <span class="summary-stat">
                                        <strong>Difference:</strong> ${Math.abs(
                                          fastestWeather.average_impact -
                                            fuelWeather.average_impact
                                        ).toFixed(1)} points
                                    </span>
                    ...
                    <div class="weather-comparison">
                        <div class="weather-route-card fastest-weather">
                            <div class="weather-route-header">
                                ...
                                <span class="weather-condition-badge ${getWeatherClass(
                                  fastestWeather.average_impact
                                )}">
                                    ${getWeatherIcon(
                                      fastestWeather.average_impact
  ...
  } else if (!includeWeather && comparisonDiv) {
    // Show message when weather is disabled
    weatherHTML = `
            <div class="weather-disabled-notice">
                <div class="notice-header">
                    <span class="notice-icon">🌤️</span>
                    <span class="notice-title">Weather Integration Disabled</span>
                </div>
                <div class="notice-content">
                    <p>Weather data is currently disabled. Enable "Include Weather Data" for detailed weather analysis including wind, waves, and visibility impacts.</p>
                    <div class="notice-actions">
                        <label class="switch">
                            <input type="checkbox" name="weather-toggle" 
                                   onchange="toggleWeatherIntegration()">
                            <span class="slider"></span>
                        </label>
                        <span>Enable Weather Analysis</span>
                    </div>
                </div>
            </div>
        `;
  } else if (includeWeather && !data.weather_recommendation && comparisonDiv) {
    // Show message when weather is enabled but no data available
    weatherHTML = `
            <div class="weather-disabled-notice">
                <div class="notice-header">
                    <span class="notice-icon">⚠️</span>
                    <span class="notice-title">Weather Data Unavailable</span>
                </div>
                <div class="notice-content">
                    <p>Weather data could not be loaded for this route. This could be due to API limitations or network issues.</p>
                </div>
            </div>
        `;
  }
  ...
  const routesAreDifferent =
    JSON.stringify(fastestPorts) !== JSON.stringify(fuelPorts);

  // Calculate differences for comparison
  let comparisonHTML = "";
  if (routesAreDifferent && fastestRoute && fuelRoute) {
    const timeDiff =
      (fuelRoute.time_hours || 0) - (fastestRoute.time_hours || 0);
    const fuelDiff =
      (fastestRoute.fuel_tonnes || 0) - (fuelRoute.fuel_tonnes || 0);
    const co2Diff =
      (fastestRoute.co2_tonnes || 0) - (fuelRoute.co2_tonnes || 0);
      ...
      comparisonHTML = `
                <div class="savings-info">
                    <p>
                        ...
                        ${
                          fuelDiff > 0
                            ? `The efficient route saves ${safeNumberFormat(
                                fuelDiff,
                                1
                              )} tonnes of fuel`
                            : fuelDiff < 0
                            ? `The fastest route uses ${safeNumberFormat(
                                Math.abs(fuelDiff),
                                1
                        ...
                        ${
                          co2Diff !== 0
                            ? ` and ${
                                co2Diff > 0
                                  ? `reduces CO₂ by ${safeNumberFormat(
                                      co2Diff,
                                      1
                                    )} tonnes`
                                  : `increases CO₂ by ${safeNumberFormat(
                                      Math.abs(co2Diff),
                                      1
                                    )} tonnes`
                              }`
                        ...
                        ${
                          timeDiff !== 0
                            ? ` while ${
                                timeDiff > 0
                                  ? `adding ${safeNumberFormat(
                                      timeDiff / 24,
                                      1
                                    )} days`
                                  : `saving ${safeNumberFormat(
                                      Math.abs(timeDiff / 24),
                                      1
                                    )} days`
                              } to the journey.`
  ...
  if (selectedHubs.length > 0) {
    const usedInFastest = selectedHubs.filter((hub) =>
      fastestPorts.includes(hub)
    );
    ...
    intermediateInfoHTML = `
            <div class="route-suggestion">
                ...
                <div class="intermediate-stats">
                    ...
                    <div class="intermediate-stat ${
                      usedInFastest.length === selectedHubs.length
                        ? "all-used"
                        : "partial-used"
                    }">
                        <strong>Fastest Route uses:</strong> ${
                          usedInFastest.length
                        }/${selectedHubs.length} ports
                        ${
                          usedInFastest.length > 0
                            ? `(${usedInFastest.join(", ")})`
                            : "None used"
                        }
                    </div>
                    <div class="intermediate-stat ${
                      usedInFuel.length === selectedHubs.length
                        ? "all-used"
                        : "partial-used"
                    }">
                        <strong>Efficient Route uses:</strong> ${
                          usedInFuel.length
                        }/${selectedHubs.length} ports
                        ${
                          usedInFuel.length > 0
    ...
    return ports
      .map((port, index) => {
        ...
        if (index === ports.length - 1)
          return `<span class="port-end">${port}</span>`;
        if (selectedHubs.includes(port))
          return `<span class="port-hub used">${port}</span>`;
  ...
  comparisonDiv.innerHTML =
    ...
    `
            ...
            <div class="route-card fastest">
                <div class="route-card-header">
                    ...
                    ${
                      fastestPorts.length > 2
                        ? '<span class="route-badge">With Intermediate Ports</span>'
                        : '<span class="route-badge">Direct Route</span>'
                    }
                    ${
                      fastestRoute.weather_impact
                        ? `
                    <span class="weather-indicator ${getWeatherClass(
                      fastestRoute.weather_impact.average_impact
                    )}">
                        ${getWeatherIcon(
                          fastestRoute.weather_impact.average_impact
                        )}
                ...
                <div class="route-path">
                    ...
                    <div class="route-stops">
                        <small>🛑 ${fastestPorts.length - 2} intermediate stop${
                            fastestPorts.length - 2 !== 1 ? "s" : ""
                          }</small>
                    </div>
                    `
                        : ""
                ...
                <div class="route-stats-grid">
                    <div class="stat-card">
                        <div class="stat-value">${safeNumberFormat(
                ...
                <div class="route-card-header">
                    ...
                    ${
                      fuelRoute.weather_impact
                        ? `
                    <span class="weather-indicator ${getWeatherClass(
                      fuelRoute.weather_impact.average_impact
                    )}">
                        ${getWeatherIcon(
                          fuelRoute.weather_impact.average_impact
                        )}
                ...
                <div class="route-path">
                    ...
                    ${
                      fuelPorts.length > 2
                        ? `
                    <div class="route-stops">
                        <small>🛑 ${fuelPorts.length - 2} intermediate stop${
                            fuelPorts.length - 2 !== 1 ? "s" : ""
                          }</small>
                    </div>
                    `
                        : ""
        ...
        ${
          ...
          (fastestPorts.length > 2 || fuelPorts.length > 2)
            ? `
        <div class="route-suggestion">
  ...
  if (fastestPorts.length > 0 && fuelPorts.length > 0) {
    ...
    const maxDistance = Math.max(
      fastestRoute.distance_km || 0,
      fuelRoute.distance_km || 0
    );
    const hubCount = Math.max(fastestPorts.length - 2, fuelPorts.length - 2, 0);

    if (hubCount > 0) {
      resultsMeta.textContent = `${totalPorts} ports • ${hubCount} hubs • ${safeNumberFormat(
        maxDistance / 1000,
        1
      )}k km`;
    } else {
      resultsMeta.textContent = `${totalPorts} ports • ${safeNumberFormat(
        maxDistance / 1000,
        1
      )}k km`;
    }
